(function(){
  ...
  var ROOM_PROFILE = 'BEDROOM REFERENCE IDENTITY LOCK — ABSOLUTE MANDATORY WHEN ROOM LOCK IS ON OR THE USER REFERS TO THE BEDROOM. Treat the uploaded bedroom photograph as the canonical identity source for THIS SAME BEDROOM, not as a loose style reference. Preserve the real room architecture, dimensions, wall positions, ceiling geometry, floor layout, bed location, headboard identity, furniture identity, furniture placement, wardrobe/dresser system, curtain location, air-conditioner location, rug zone, established color palette, material families, and spatial relationships. Do not redesign, modernize, beautify, simplify, enlarge, shrink, clean into a showroom, replace furniture, move major furniture, invent extra windows or doors, or transform the room into a hotel, catalog, luxury interior, or generic bedroom. CAMERA FLEXIBILITY — IMPORTANT. The bedroom identity is fixed, but selfie camera angle, camera height, crop, orientation, distance, and visible room coverage may change according to the selected bedroom selfie pose and angle. Reconstruct newly visible areas conservatively from the established geometry rather than inventing a different room. REFERENCE LIGHTING IS NOT LOCKED. Do not copy the reference photograph lighting merely because it is visible in the reference. The dedicated Bedroom Lighting control is the sole authority for active light sources, direction, intensity, color temperature, falloff, shadows, reflections, exposure, and low-light behavior.';
  ...
  var ROOM_COMPLIANCE = 'BEDROOM ROOM-REALISM COMPLIANCE CHECK — REQUIRED BEFORE RENDERING. Verify that the result still depicts the SAME canonical bedroom with unchanged architecture and major furniture relationships; the selected Bedroom Lighting control, not reference-image lighting, determines illumination; the selected Bedroom Clutter control determines any extra disorder; bedding, curtains, walls, wood, mirrors, glass, metal, plastic, cables, and small objects obey real material and contact physics; mirrors and reflections remain geometrically consistent; no object floats, duplicates, melts, clips, or loses a logical support/contact relationship; and ordinary imperfections remain subtle rather than staged. If realism conflicts with room identity or an explicit bedroom control, preserve the room identity and explicit control and revise only unspecified micro-details.';
  ...
  function installToggle(){
    ...
    function sync(){
      ...
      txt.textContent=v.roomLock?'مفعّل: ثبات هندسة وهوية الغرفة والمواد الأساسية، بينما الإنارة والفوضى تتبع خانات غرفة النوم.':'متوقف: عند ذكر غرفة النوم يستخدم التطبيق نفس الغرفة المرجعية، مع بقاء الإنارة والفوضى تحت تحكم خاناتهما.';
    }
  ...
  window.buildFinal=function(){
    ...
    var v=rawState();
    if(roomRequested(v)){
      return [ROOM_PROFILE,ROOM_PROFILE_DETAILS,MATERIAL_RULE,CONTACT_RULE,CLUTTER_RULE,ROOM_CAMERA_REALISM,base,ROOM_COMPLIANCE].join('\n\n');
    }
    return base;
  };
