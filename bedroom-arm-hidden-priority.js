(function(){
  ...
  var GEOMETRY='BEDROOM SELECTED POSE AND ANGLE LOCK — HIGHEST PRIORITY. The bedroom pose and selfie camera angle selected in the user-facing controls are explicit hard constraints. Preserve exactly one selected pose and exactly one compatible selected angle. Do not replace the selected angle with an automatically preferred angle and do not combine high-angle, low-angle, eye-level, overhead, or side-bed-level viewpoints in one image. Camera-to-face distance, tiny roll, exact crop, and exact hidden phone position may adapt automatically only to make the selected pose and selected angle physically possible. Mirror-selfie behavior is excluded from this hidden-arm bedroom mode.';

  var RULE='TRUE SELF-TAKEN BEDROOM SELFIE — HIGHEST PRIORITY FOR CHATGPT AND GEMINI. The final image MUST be an actual self-taken front-camera selfie: the person is physically photographing himself with the smartphone that he personally holds in the camera-holding hand. The camera viewpoint must originate from a physically reachable phone position created by a naturally extended camera-holding arm, exactly as in a real handheld selfie. This must never read as a third-person portrait, another person taking the photo, a tripod shot, a remotely positioned camera, or a professional photographer composition unless the user explicitly requests such a different capture method. Engineer the hidden selfie biomechanics realistically from the selected bedroom pose: preserve natural shoulder socket position, clavicle alignment, upper-arm length, elbow mechanics, forearm length, wrist orientation, and believable phone reach. Arm extension must be appropriate to the automatically resolved camera-to-face distance; it may retain a natural elbow bend rather than becoming rigidly straight. Perspective and framing must clearly feel like a real person holding a phone at selfie distance. CAMERA-HOLDING ARM VISIBILITY OVERRIDE — ABSOLUTE. Although the camera-holding arm is physically extended to take the selfie, the camera-holding upper arm, forearm, elbow, wrist, hand, fingers, and phone must remain completely outside the captured image, including every edge and corner. Any instruction anywhere in this prompt that mentions natural arm extension, selfie reach, foreshortening, or the person holding the phone must be interpreted as real hidden off-frame biomechanics only, never as permission to show the camera-holding limb or phone. If the selected selfie angle or composition would expose any part of that limb or phone, adjust only the virtual phone position, camera-to-face distance, crop, field coverage, or amount of torso/background shown until the entire camera-holding limb and phone remain outside the image. Never solve composition by stretching, shortening, enlarging, telescoping, rigidly straightening, detaching, blurring away, or otherwise distorting human anatomy. The result must unmistakably feel like a genuine self-taken smartphone selfie while keeping the entire camera-holding limb and phone outside the frame.';

  var CHECK='TRUE SELFIE MECHANICS COMPLIANCE CHECK — REQUIRED IMMEDIATELY BEFORE RENDERING. Confirm all of the following simultaneously: (1) the person is clearly photographing himself rather than being photographed by someone else or by a remote/tripod camera; (2) the virtual phone position is physically reachable through a naturally extended camera-holding arm with realistic shoulder, elbow, forearm, and wrist biomechanics appropriate to the selected pose and resolved camera distance; (3) not a single visible portion of the camera-holding upper arm, elbow, forearm, wrist, hand, fingers, or phone enters the final image; (4) the camera perspective still reads naturally as a handheld front-camera selfie; and (5) the exact selected bedroom pose and exact selected compatible selfie angle are both preserved. If any condition fails, revise only the virtual camera placement, distance, crop, or unspecified composition details before rendering. Do not alter identity, body proportions, clothing, selected pose, or selected angle.';
