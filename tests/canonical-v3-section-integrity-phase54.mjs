...
for(const [section,extra] of Object.entries(cases)){
  ...
  if(section==="car") assert.equal(out.phase54.allCommonFieldsRouted,false,"car: raw generic field evidence must stay isolated");
...
assert.match(ui,/const carInterior=ui\.dedicatedControls===["']carInterior["']/iu,"UI must detect the car-interior section");
assert.match(ui,/#post-processing-panel[\s\S]{0,100}hidden:carInterior[\s\S]{0,60}disabled:carInterior/iu,"post-processing remains blocked for car interior");
assert.match(ui,/#hair[\s\S]{0,80}hidden:false[\s\S]{0,40}disabled:false/iu,"hair must stay active in car interior");
assert.match(ui,/#skin[\s\S]{0,80}hidden:false[\s\S]{0,40}disabled:false/iu,"skin must stay active in car interior");
assert.match(ui,/realism-core-title[\s\S]{0,120}hidden:false[\s\S]{0,40}disabled:false/iu,"realism core must stay active in car interior");
assert.match(ui,/advanced-realism-title[\s\S]{0,120}hidden:false[\s\S]{0,40}disabled:false/iu,"advanced realism must stay active in car interior");
assert.match(ui,/context-secondary-panel[\s\S]{0,80}hidden:false[\s\S]{0,40}disabled:false/iu,"context controls must stay active for through-glass realism");
...
console.log("PHASE54_CAR_RAW_FIELD_EVIDENCE=false");
console.log("PHASE54_CAR_SUBJECT_CONTROLS=active");
console.log("PHASE54_CAR_THROUGH_GLASS_REALISM=active");
