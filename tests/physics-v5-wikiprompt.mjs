...
assert.match(rightSidePack.positive, /sole identity source\./u);
assert.match(rightSidePack.positive, /Describe only anatomy that naturally enters this crop/u);
...
assert.doesNotMatch(carPack.positive, /Selected angle intent:/u, "Driver prompts must not retain a second generic angle description");
assert.doesNotMatch(carPack.positive, /For a custom scene, preserve the user's requested place/u, "Car prompts must not retain dormant custom-scene authority text");
