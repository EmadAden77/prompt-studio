...
assert.equal((sofaSource.match(/\{ id:"[a-z]{2}_[a-z_]+"/g) || []).length, 13, "expected 13 sofa templates");
...
assert.ok(promptSource.indexOf("this.sofaTemplateText(c)") < promptSource.indexOf("SOFA_GROUNDING_LOCK"), "sofa template must precede sofa grounding lock in prompt assembly");
assert.ok(promptSource.indexOf("SOFA_GROUNDING_LOCK") < promptSource.indexOf("GROUNDING.sitting"), "sofa grounding must precede generic sitting grounding");
