...
const engineGate=fs.readFileSync(new URL("../js/canonical/engine-gate.js",import.meta.url),"utf8"); assert.match(engineGate,/from\s+["']\.\/canonical-v3-phase50\.js["']/u,"live engine gate must use the current Phase 50 output while Phase 46 remains a historical behavior contract");
console.log(`PHASE46_CAREXTERIOR_NIGHT_WORDS=${words(carVilla.prompt)}`); console.log(`PHASE46_CAREXTERIOR_NIGHT_SAMPLE=${carVilla.prompt}`); console.log("PHASE46_DETERMINISM=10/10"); console.log("✓ Phase 46 natural prompt language passed under Phase 50 live gate");