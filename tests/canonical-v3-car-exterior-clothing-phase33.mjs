...
assert.ok(words(thobePrompt) <= 250, `thobe carExterior prompt exceeds 250 words (${words(thobePrompt)})`);
...
assert.ok(words(outfitPrompt) <= 250, `full-outfit carExterior prompt exceeds 250 words (${words(outfitPrompt)})`);
...
assert.ok(words(customPrompt) <= 250, `custom carExterior prompt exceeds 250 words (${words(customPrompt)})`);

assert.deepEqual(
  thobeOutputs[0].canonical.hard_constraints,
  outfitOutputs[0].canonical.hard_constraints,
  "carExterior clothing selection must not mutate hard constraints"
);
assert.deepEqual(
  customOutputs[0].canonical.hard_constraints,
  outfitOutputs[0].canonical.hard_constraints,
  "custom clothing must not mutate hard constraints"
);
...
console.log("✓ Phase 33 wide carExterior clothing with thobe and shemagh contracts passed");
