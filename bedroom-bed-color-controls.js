(function(){
  ...
  function arrangeUI(){
    ...
    if(n)n.textContent='اختر الزاوية، الوضعية، تعبير الوجه، اليد الأخرى، الملابس، الإضاءة، الفوضى، حالة السرير، واقعية حالة الصورة، وألوان الصورة. بقية هندسة السيلفي تُبنى تلقائيًا مع قفل الهوية.';
  }
  ...
  function master(){
    var s=S();
    return 'BEDROOM TEN-CONTROL MASTER SYSTEM — ABSOLUTE HIGHEST PRIORITY. Preserve exactly these ten user-facing bedroom controls: selfie angle, person pose, facial expression, free-hand pose, clothing, bedroom lighting, bedroom clutter, bed condition, image-condition realism, and color response. BED CONDITION controls only bedding arrangement and must not add or remove room clutter. COLOR RESPONSE controls recorded color only and must not change the selected physical light source, time logic, identity, or room materials.\n\nACTIVE BED CONDITION: '+String(s.bedroomBedCondition||BED_DEFAULT)+'. ACTIVE COLOR RESPONSE: '+String(s.bedroomColorTone||COLOR_DEFAULT)+'.';
  }
