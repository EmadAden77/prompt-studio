...
- `streetMood="auto"` resolves from `streetHour` through `HOUR_TO_MOOD`.
- Only resolved `scene.facts.street_mood` is stored; no hour value is stored in Canonical V3.
- Manual fixed moods, including `dust`, `souq`, and `event`, override auto resolution.
- `street_mood` is reserved from generic scene-fact rendering and consumed by `describeSaudiStreetRealism()`.
- Auto hour 7 emits three rush details; auto hour 21 emits three cafe details.
- Golden prompts and new mood prompts remain **≤250 words**, deterministic **10/10**, with hard constraints unchanged.
