...
export function describeNightPhysics(canonical, raw = {}, sectionId = "") {
  ...
  const sensorRule="Low-light phone exposure uses raised ISO with subtle grain, shadow noise, and mild loss of fine detail; skin remains textured.";
  const motionRule=motion ? "Natural movement adds slight blur to the moving hand or loose hair strands; passing car lights can streak briefly." : "Moving cars can leave short light streaks.";
  ...
  const exposureRule=flash
    ...
    : "Exposure favors a clearer face with darker background or visible background lights with a dimmer face, never both perfectly bright.";
  const nightGuard="Night processing may lift shadows slightly, but darkness remains visibly nocturnal and never turns the scene into daylight.";
