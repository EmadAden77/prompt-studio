...
function mountCarExteriorControls() {
  ...
  const clothing = makeSelect("car-exterior-clothing", "carExteriorClothing", "الملابس", SCENES.carExterior?.clothing ?? []);
...
function exposeSceneSelectForDailyScenes() {
  ...
  if (["solo", "group"].includes(section)) {
    repopulateSceneSelect(field.dataset.phase22Selected || document.querySelector("#scene")?.value || "bedroom");
    field.hidden = false;
...
export function installPhase22UI() {
  ...
  document.addEventListener("change", (event) => {
    if (event.target?.id === "scene") {
      ...
      setTimeout(() => repopulateSceneSelect(field?.dataset.phase22Selected || event.target.value), 0);
    }
