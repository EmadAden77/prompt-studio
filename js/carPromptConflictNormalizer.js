...
- The vehicle is one 2017 RANGE ROVER SPORT AUTOBIOGRAPHY DYNAMIC · FUJI WHITE. Preserve every vehicle feature visibly established by IMAGE A before using model knowledge for anything unseen.
...
const RANGE_ROVER_COMPLETION_LOCK = `VEHICLE IDENTITY & UNSEEN-AREA COMPLETION LOCK — 2017 RANGE ROVER SPORT AUTOBIOGRAPHY DYNAMIC · FUJI WHITE
- Treat the cabin as one continuous real 2017 Range Rover Sport Autobiography Dynamic, not as a front-seat set with an invented rear half.
...
- When a selected selfie angle reveals previously unseen passenger-side or rear-cabin areas, continue them only as necessary using authentic 2017 Range Rover Sport Autobiography Dynamic architecture consistent with the visible configuration and the same design language.
- Continue the visible light-Ivory perforated leather family, matching stitching logic, compatible door-panel construction, trim language, roof/headliner treatment, glazing, pillar proportions, seat geometry, rear seating, rear door geometry, rear console/vent treatment, and cargo/rear-cabin spatial proportions as one coherent vehicle.
...
- If any exterior bodywork becomes visible from an open sightline, mirror, window edge, or unusual but physically reachable cabin angle, the vehicle body is WHITE and remains the same 2017 Range Rover Sport Autobiography Dynamic.
...
function normalizeUnifiedCarReference(text) {
  ...
  output = output.replace(
    ...
    "Generate one new, ordinary, photorealistic smartphone selfie inside the exact same 2017 Range Rover Sport Autobiography Dynamic in Fuji White represented by IMAGE A. IMAGE A is the single unified reference for both the subject identity and the vehicle."
  );
  ...
  output = output.replace(
    ...
    "IMAGE A is the single unified subject-and-vehicle reference. The vehicle is a 2017 Range Rover Sport Autobiography Dynamic in Fuji White. Preserve every visible cabin detail from IMAGE A exactly as supported by the image; use the model identity only to continue areas that the selected camera angle newly reveals."
  );
  ...
  if (!output.includes("VEHICLE IDENTITY & UNSEEN-AREA COMPLETION LOCK — 2017 RANGE ROVER SPORT AUTOBIOGRAPHY DYNAMIC · FUJI WHITE")) {
    output = output.replace(
...
function syncUnifiedCarUI() {
  ...
  if (region) region.textContent = "2017 RANGE ROVER SPORT AUTOBIOGRAPHY DYNAMIC · FUJI WHITE · SINGLE IMAGE A";
  const name = document.querySelector("#sceneName");
  if (name) name.textContent = "رنج روفر 2017 Fuji White — نفس الشخص ونفس السيارة";
  ...
  if (reasons) reasons.replaceChildren(
    ...
    reason("إذا ظهرت الخلفية الداخلية أو جهة الراكب من زاوية جديدة، تُستكمل كجزء من نفس Range Rover Sport Autobiography Dynamic 2017 وبنفس لغة التصميم والخامات."),
  ...
  if (send) send.innerHTML = "أرفق <strong>صورة واحدة فقط</strong> باعتبارها <strong>IMAGE A</strong>: الشخص داخل نفس السيارة. لا ترفق IMAGE B لقالب السيارة. IMAGE A هي سلطة الهوية والسيارة معًا، والسيارة Range Rover Sport Autobiography Dynamic 2017 بيضاء؛ الأجزاء غير الظاهرة تُستكمل فقط عند الحاجة وبنفس تصميم السيارة.";
  ...
  if (summary) {
    ...
    summary.textContent = `🚙 ${templateName}${timeName ? ` · ${timeName}` : ""} · مرجع واحد IMAGE A · Range Rover Sport Autobiography Dynamic 2017 أبيض · Xiaomi 15 Ultra Front Camera · متوقفة في السعودية`;
  }
  ...
  if (hubTitle) hubTitle.textContent = "IMAGE A — الشخص + Range Rover Sport Autobiography Dynamic 2017";
  const hubMeta = document.querySelector(".car-template-card__preview small");
  if (hubMeta) hubMeta.textContent = reference.src
    ? "مرجع واحد مرفوع: الهوية والسيارة معًا. الظاهر ثابت، والخلف/الأجزاء غير الظاهرة تُستكمل بنفس تصميم Range Rover Sport Autobiography Dynamic 2017 عند الحاجة."
