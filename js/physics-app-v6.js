...
function readState() {
  return {
    ...
    subjectMoment:value("subject-moment"), interactionObject:value("interaction-object"), hasReference
  };
}

function normalizeEnhancedState(rawState = {}) {
  const base = normalizeState({ ...REALISM_DEFAULTS, ...rawState });
  const resolved = resolveRealismCoreState(base);
  const renormalized = normalizeState(resolved.state);
  return { state:{ ...renormalized, ...resolved.state }, conflicts:resolved.conflicts };
}
...
function buildEnhancedPack(rawState = {}) {
  ...
  const phoneAnchor = "\n\n[PHONE REALISM]";
  const positive = base.positive.includes(phoneAnchor)
    ? base.positive.replace(phoneAnchor, `\n\n${coreSections}${phoneAnchor}`)
    : `${base.positive}\n\n${coreSections}`;
  const negative = [...new Set([...base.negative.split(/,\s*/), ...REALISM_CORE_NEGATIVE_RULES])].join(", ");
  return {
    ...base,
    state:base.state,
    positive,
    negative,
    qa:[...base.qa, ...realismCoreQaItems(base.state, conflicts)],
    conflicts
  };
}
...
function refreshDynamicFields() {
  ...
  const { state:normalized } = normalizeEnhancedState(readState());
...
function renderPrompt() {
  ...
  resultMeta.textContent = `${pack.template.title} · Xiaomi 15 Ultra Front · REALISM CORE · ${pack.state.time === "night" ? "ليلي" : "نهاري"}`;
...
function downloadPrompt() {
  const pack = ["WIKIPROMPT SELFIE STUDIO · REALISM CORE", "", "POSITIVE PROMPT", positivePrompt.value, "", "NEGATIVE PROMPT", negativePrompt.value].join("\n");
  ...
  link.href = href; link.download = "wikiprompt-selfie-realism-core.txt"; document.body.append(link); link.click(); link.remove(); URL.revokeObjectURL(href);
...
["time","pose-family","pose","car-seat","lighting","clothing","fabric","composition","selfie-angle","place-state","people-density","subject-moment"].forEach((id) => {
